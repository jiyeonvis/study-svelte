...
<div class="section">
	<div class="brief-box box">
		<p class="title"> 서중회(序中會),<br class="break"> <span class="title2">조선일보 인맥 네트워크의 심장</span></p>
		<p class="brief"> 방일영장학회 선배들의 모임 ‘서중회’. 조선일보를 창간한 계초 방응모가 1928년 설립한 ‘춘해장학부'가 서중회의 모태다.
			1974년 이후 지금까지 763명이 방일영장학회의 장학금 수혜를 받았다.
			조선일보 방상훈 사장은 2020년 서중회 신년회 및 정기총회에서 “조선일보가 100년을 이어오며 1등 언론의 위치를 굳건히 지킬 수 있었던 데에는
			사회 곳곳에서 중추적인 역할을 맡고 계신 서중회원 여러분의 진심 어린 성원이 많은 도움이 되었다”라고 말했다. 서중회, 그들은 누구일까?
			뉴스타파는 스스로 권력이 된 조선일보의 인맥 네트워크 핵심, 방일영장학회 졸업생 모임 서중회 주요 회원 명단과 그들의 이력을 공개한다.</p>
	</div>
	...
	<p class="source">사진 출처 : 법률신문 DB, 조선일보 인물정보 <br>
	데이터 시각화 : 김지연 | 데이터 : 최윤원 </p>
...
<style lang="scss" >
	...
	@mixin mobile {
		@media (max-width: #{$breakpoint-tablet - 1px}) { //min-width: #{$breakpoint-mobile}) and 
			@content;
		}
	} 
	@mixin tablet {
	...
	.button {
		...
		// 	}
		// }
		&.cate1 { background-color : $c-cate1; }
		&.cate2 { background-color : $c-cate2; }
		&.cate3 { background-color : $c-cate3; }
		&.cate4 { background-color : $c-cate4; }
		&.cate5 { background-color : $c-cate5; }
		&.cate6 { background-color : $c-cate6; }
